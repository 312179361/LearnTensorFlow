...
x_test = tf.convert_to_tensor(x_test, dtype=tf.float32) / 255.  #这样x的数值范围0~1
...
for epoch in range(10):
    ...
    for step, (x, y) in enumerate(test_db):
        ...
        # out:[b, 10] 属于实数范围，转换成[0,1]的范围。利用softmax
...
sigmoid() 可以将矩阵中所有的元素，从实数映射到[0,1]中，

softmax() 可以将矩阵中所有的元素，从实数映射到[0,1]中。并且所有元素相加等于1

tanh() 可以将矩阵中所有的元素，从实数映射到[-1,1]中，
...
print(prob)
# 相加，验证下是不是约为1
total = tf.reduce_sum(prob, axis=1)
print(total)

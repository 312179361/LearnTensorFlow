'''
利用kreas 的compile&fit对原有的流程进行简化
...
# 以上的模型训练和模型测试可以用compile_fit的方法简写
# 参数optimizer指定优化器
# 参数loss指定loss的方式，是MSE还是交叉熵
# 参数metrics是指定测试的方式，即测试的指标
...
# 参数validation_freq 训练多少次epoch，做一次测试，

model.fit(db, epochs=5, validation_data=db_test, validation_freq=2) # 即训练两次epoch就进行一次测试

# 最终测试，这个和上面的validation_freq是一样的，区别在于上面的是在训练中间进行测试
# 这个evaluate是在最后才执行，这里面的db_test可以用一个全新的数据集进行最终的测试
...
x = sample[0]
y = sample[1] # 在预测时。y可能没有

# 将x代入预测
pred = model.predict(x)
# pred = model(x) # 和上面一行一样，都是代入进行预测
